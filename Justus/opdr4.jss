...
(function() {
	...
	geolocatie.position = function() {
		this.update = function() {						// Vraag de huidige positie aan geo.js, stel een callback in voor het resultaat
			intervalCounter++;
			geo_position_js.getCurrentPosition(_set_position, _geo_error_handler, {enableHighAccuracy:true});
		}
		
		this.set = function(position) {					// Callback functie voor het instellen van de huidige positie, vuurt een event af
	...
	geolocatie.map = function() {
		this.generate = function(myOptions,canvasId) {
		...
		this.calculateDistance = function(p1,p2) {		// Bereken het verchil in meters tussen twee punten
			var pos1 = new google.maps.LatLng(p1.coords.latitude, p1.coords.longitude);
			var pos2 = new google.maps.LatLng(p2.coords.latitude, p2.coords.longitude);
			return Math.round(google.maps.geometry.spherical.computeDistanceBetween(pos1, pos2), 0);
		}
		this.checkLocations = function(event) {			// Controleer de locaties en verwijs naar een andere pagina als we op een locatie zijn
	...
	geolocatie.debug = function() {
		geolocatie.debug.prototype.geoError = function(code, message) {
			debug_message('geo.js error '+code+': '+message);
		}
		geolocatie.debug.prototype.message = function(message) {
			(customDebugging && debugId)?document.getElementById(debugId).innerHTML:console.log(message);
		}
		geolocatie.debug.prototype.customById = function(debugId) {
...
/////////////////////////////////////////////////////////////////////////////////////////
// NOG NIET GEIMPLEMENTEERDE CODE!!/////////////////////////////////////////////////////
